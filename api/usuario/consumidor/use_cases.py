...
class ConsumidorUseCase:
    @staticmethod
    async def create_consumidor(
        db: AsyncSession, data: ConsumidorAuth
    ) -> Optional[Consumidor]:
    ...
    @staticmethod
    async def create_consumidor_google(
        db: AsyncSession, data: ConsumidorGoogle
    ) -> Optional[Consumidor]:
        ...
        try:
            ...
            _usuario_google = (
                await usuarioauth_manager.get_usuario_auth_google_by_id_google(
                    data.id_google
                )
            )
            if _usuario_google:
                ...
                _consumidor = await consumidor_manager.get_consumidor_by_email(
                    data_user.get("email")
                )
                if _consumidor:
                    raise HTTPException(
                    ...
                    _new_consumidor = await consumidor_manager.create_consumidor(
                        _consumidor_auth_google
                    )
                    ...
                    _auth_google = await usuarioauth_manager.create_usuario_auth_google(
                        _data_auth_google
                    )
    ...
    async def update_consumidor_data(
        ...
        consumidor_manager = ConsumidorManager(db=db)
        _consumidor_exists = await consumidor_manager.get_consumidor_by_id(
            id_consumidor
        )
        ...
        if erros:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=erros)
    ...
    async def delete_consumidor(db: AsyncSession, id_consumidor: UUID):
        consumidor_manager = ConsumidorManager(db=db)
        _consumidor_exists = await consumidor_manager.check_consumidor_exists(
            id_consumidor
        )
        if not _consumidor_exists:
            raise HTTPException(
    ...
    async def restore_consumidor_by_email(db: AsyncSession, email: EmailStr):
        consumidor_manager = ConsumidorManager(db=db)
        _consumidor_exists = await consumidor_manager.check_deleted_consumidor_exists(
            email
        )
        if not _consumidor_exists:
            raise HTTPException(
